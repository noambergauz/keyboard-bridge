...
class KeyboardBridgeDaemon:
    ...
    def __init__(self, port: int = 8080, device_id: int = 1, host: str = 'localhost', keymap: Optional[dict] = None):
        self.port = port
    ...
    async def handle_keyboard_event(self, client: ClientConnection, data: dict):
        ...
        try:
            ...
            if processed_event:
                ...
                if keyboard:
                    ...
                    logger.debug(f"Sent key event to virtual keyboard: {processed_event}")
                else:
    ...
    async def handle_composition_event(self, client: ClientConnection, data: dict):
        ...
        try:
            ...
            if processed_event:
                ...
                if keyboard:
                    ...
                    logger.debug(f"Sent composition event to virtual keyboard: {processed_event}")
                else:
                    logger.debug(f"Mock mode: received composition event: {processed_event}")
                    
        except Exception as e:
            logger.error(f"Error handling composition event: {e}")
            
    async def handle_connect(self, client: ClientConnection, data: dict):
        ...
        try:
            ...
            logger.info(f"Client {client.client_id} assigned to device {self.device_id}")
            
        except Exception as e:
            logger.error(f"Error handling connect event: {e}")
            
    async def handle_keymap(self, client: ClientConnection, data: dict):
        ...
        try:
            ...
            if keyboard:
                ...
                self.virtual_keyboards[self.device_id] = VirtualKeyboardDevice(self.device_id, keymap=keymap)
        except Exception as e:
